%% -------------------- 1 vs 5 GROUP ANALYSIS ----------------------- %

% This script runs first (individual subject) and second (group) level  MVPA on 
% passive and active finger stimulation. This strategie (1 vs. 5) uses a two
% class decoder to tell the thumb from the little finger. For details of the 
...
path_1vs5_output = ''; % >>> SET directory to save results. 
...
%% main settings
cfg = [];
cfg.class_method = 'AUC';      % decoding measure: AUC
...
cfg.outputdir = strcat(path_1vs5_output, '/a_left');  % where the output is generated
...
cfg.outputdir = strcat(path_1vs5_output, '/p_left');  % where the output is generated
cfg.crossclass = 'yes';            % time-by-time temporal generalization 
cfg.channels = 'ALL_NOSELECTION';  % for feature selection
...
class_spec{2} = '10';   % little finger 
cfg.class_spec = class_spec;   % specification of stimulus classes

% ACTIVE 
cfg.datadir = path_active_correct  % path to preprocessed data files
cfg.filenames = right_active;  % list of files to be analyzed
cfg.outputdir = strcat(path_1vs5_output, '/a_right'); % where the output is generated
cfg.crossclass = 'yes';            % time-by-time temporal generalization
cfg.channels = 'ALL_NOSELECTION';  % for feature selection
...
cfg.outputdir = strcat(path_1vs5_output, '/p_right');  % where the output is generated
cfg.crossclass = 'yes';            % time-by-time temporal generalization 
cfg.channels = 'ALL_NOSELECTION';  % for feature selection
...
func.organize_output(path_1vs5_output)

%% main settings

cfg.startdir = path_1vs5_output;     % 1st lvl results folder
...
cfg.reduce_dims = 'diag';            % compute time-by-time: extracts the entire TGM, 
...
cfg.acclim2D = [.46 .72];  % modifies the range of accuracy values
